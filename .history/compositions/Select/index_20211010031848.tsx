...
const Picker = ({ }) => {
    ...
    type AnimalName = 'redPanda' | 'cat' | 'dog' | 'aardvark' | 'snake'
    type AnimalInfo = {
        index: number;
        value: string; 
    }

    let keys: Record<AnimalName, AnimalInfo> = {  
        redPanda: { index: 0, value: 'Red Panda' },
        cat: { index: 1, value: 'Cat' },
        dog: { index: 2, value: 'Dog' },
        aardvark: { index: 3, value: 'Aardvark' },
        snake: { index: 4, value: 'Snake' }
    }

    return (
        <Select label={label}>
            {keys.collection((key: Key, index: number) => (
                <div key={key}>
