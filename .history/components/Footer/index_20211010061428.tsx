...
const UserSession = ({ 
    status, userCount, roomId }: { status: string; userCount: number; roomId: string; }) => (
    <Text>
     {status === 'connected'
...
export const Footer = ({ editor, user, status, setName }: FooterProps) => (
    <>
        <FooterTopBorder /> 
        <StyledFooter> 
        ...
        <StyledFooter>
            <UserSession status={status} userCount={userCount} roomtId={roomId} />

            <UserAccount user={user} update={setName} /> 
        </StyledFooter>
    </>
);
