...
export const AlignmentHandlers = ({ editor }: { editor: Editor }) => {
    ...
    const alignRight = () => editor.chain().focus().setTextAlign('left').run()
    ...
    const handleAlign = (index: number) => {
        ...
        else if(index===1) return alignRight()
        else if(index===2) return alignCenter()
        return alignJustify()
    }
    ...
    return (<> 
        {alignmentIcons.map((item: IconMapping, index: number) => (
            <MenuBarItem
                ...
                width={'24'} 
                color={'purple'}
                action={() => editor.chain().focus().setTextAlign(`${item.alignment}`)}
                isActive={() => true}
                isDisabled={() => false}
            />
        ))}
