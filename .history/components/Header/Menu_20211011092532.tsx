...
import { useComboBox, useFilter, useButton, useHover, mergeProps } from 'react-aria'
...
interface ExtendedInputGroupProps<T> {
    ...
    focused: boolean; 
    hovered: boolean;
    otherProps: ComboBoxProps<T>; 
}

const ExtendedInputGroup<T></T> = React.forwardRef<
    React.ElementRef<typeof InputGroup>,
    Radix.ComponentPropsWithoutRef<typeof InputGroup> & ExtendedInputGroupProps<T>
>((props, forwardedRef) =>{

    let { inputProps, buttonProps, hoverProps, inputRef, buttonRef, icon, focused, hovered, ...otherProps} = props
