...
const Search = ({ editor }: { editor: Editor }) => {
    ...
    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => setQuery(event.currentTarget.value); 

    return (
        <Flex style={{ fd: 'row', jc: 'flex-start', ai: 'center', gap: '$2', margin: 0, padding: 0 }}>
            <input 
                type='search'
                value={query} 
                ...
                style={{ width: '100px', maxWidth: '150px', textOverflow: 'ellipses' }}
            /> 
            <button onClick={() => alert(`The query is ${query}`)}> 
                go 
            </button>
        </Flex>
    );
