...
export const MenuBarItem = ({ 
    ...
    return (
        <MenuItemButton {...hoverProps} onClick={action} css={{ border: '2px solid', borderColor: isHovered ? 'red' : '$border' }}>{        wrapSvg 
            ?   typeof icon === 'string' ?   title :   icon
            :   <IconWrapper width={height} height={width} children={icon} color={color} />
        } </MenuItemButton>
    );
}
