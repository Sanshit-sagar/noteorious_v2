...
import { Picker } from '../../compositions/Select' 
...
const MenuBarSelector = ({ icon, title, action, isActive = false, isDisabled = false, isHovered , type = 'select', selectProps }) => {
    return (
        <MenuBarItem>
            {icon || title}
        </MenuBarItem>
    ...
    return (
        <StyledHeader>
            ...
            {items.map((item: IMenuBarItem, index: number) => (
                <Fragment key={index}>
                    {   
                            (!item?.type?.length || item?.type === 'button')  ?   <MenuBarItem {...item} />
                        :   (item?.type === 'select')  ?   <MenuBarSelector {...item} />
