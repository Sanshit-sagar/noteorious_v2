...
const CommandsList = ({ items, selectedIndex, selectItem }: CommandsListProps) => {
    ...
    const selectItem = (index: number) => items[index] ? command(items[index]) : console.log('..')

    const onKeyDown = ({ event }: { event: KeyboardEvent }) => {
        if (event.key !== 'ArrowUp' && event.key !== 'ArrowDown' && event.key !== 'Enter') {
            return false;
        } 
        if (event.key === "ArrowUp") upHandler(); 
        if (event.key === "ArrowDown") downHandler(); 
        if (event.key === "Enter") enterHandler();  
        return (event.key !== 'ArrowUp' && event.key !== 'ArrowDown' && event.key !== 'Enter') ? false : true
    }
