...
const CommandsList = ({ items, command }: CommandsListProps) => {
    ...
    const onKeyDown = ({ event }: KbeProps): boolean => {
        if (isUpwards(event)) upHandler()
        if (isDownwards(event)) downHandler() 
        if (isReturnKey(event)) enterHandler()
        return ((isUpwards(event)) || (isDownwards(event)) || (isReturnKey(event)))
    }
