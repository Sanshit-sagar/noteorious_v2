...
>((contentProps, forwardedRef) => {
    let { sideOffset, children, ...props }: ExtendedContentProps = contentProps
    return (
        <StyledContent 
            {...props}  
            ref={forwardedRef}
...
export const PopoverContent = ({ children, hideArrow, ...props }: { children: React.ReactNode; hideArrow: boolean; props: PopoverContentOwnProps}) => (
    <ExtendedContent sideOffset={2} {...props} ref={fowardedRef}>
        <>
        {children}
        {!hideArrow && (
            <Box css={{ color: '$panel' }}>
